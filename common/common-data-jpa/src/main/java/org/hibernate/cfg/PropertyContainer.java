...
class PropertyContainer {
    ...
    PropertyContainer(XClass clazz, XClass entityAtStake, org.hibernate.cfg.AccessType defaultClassLevelAccessType) {
        ...
        this.collectPersistentAttributesUsingLocalAccessType(this.persistentAttributeMap,
                persistentAttributesFromGetters, fields, getters);
        this.collectPersistentAttributesUsingClassLevelAccessType(this.persistentAttributeMap,
                persistentAttributesFromGetters, fields, getters);
    }
    ...
    private void collectPersistentAttributesUsingLocalAccessType(Map<String, XProperty> persistentAttributeMap,
        ...
        while (propertyIterator.hasNext()) {
            ...
            if (localAccessAnnotation != null && localAccessAnnotation.value() == javax.persistence.AccessType.PROPERTY) {
                ...
                if (previous != null) {
                    throw new MappingException(LOG.ambiguousPropertyMethods(this.xClass.getName(),
                            HCANNHelper.annotatedElementSignature(previous),
                            HCANNHelper.annotatedElementSignature(xProperty)), new Origin(SourceType.ANNOTATION,
                            this.xClass.getName()));
                }
    ...
    private void collectPersistentAttributesUsingClassLevelAccessType(Map<String, XProperty> persistentAttributeMap,
                                                                      Map<String, XProperty> persistentAttributesFromGetters, List<XProperty> fields, List<XProperty> getters) {
    ...
    private void assertTypesAreResolvable() {
        ...
        String msg = "Property " + StringHelper.qualify(this.xClass.getName(), xProperty.getName()) + " has an " +
                "unbound type and no explicit target entity. Resolve this Generic usage issue or set an explicit " +
    ...
    private org.hibernate.cfg.AccessType determineLocalClassDefinedAccessStrategy() {
        ...
        org.hibernate.annotations.AttributeAccessor accessType =
                this.xClass.getAnnotation(org.hibernate.annotations.AttributeAccessor.class);
        if (accessType != null) {
        ...
        if (hibernateDefinedAccessType != org.hibernate.cfg.AccessType.DEFAULT && jpaDefinedAccessType != org.hibernate.cfg.AccessType.DEFAULT && hibernateDefinedAccessType != jpaDefinedAccessType) {
            throw new org.hibernate.MappingException("@AccessType and @Access specified with contradicting values. " +
                    "Use of @Access only is recommended. ");
        } else {
