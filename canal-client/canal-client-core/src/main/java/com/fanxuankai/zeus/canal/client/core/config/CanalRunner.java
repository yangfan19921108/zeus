...
public class CanalRunner implements ApplicationRunner {
    ...
    private final ScheduledExecutorService scheduledExecutor = new ScheduledThreadPoolExecutor(1,
            new ThreadFactoryBuilder().setNameFormat("CanalRunner-%d").build());
    ...
    public CanalRunner(Config config) {
        this.config = config;
    }
    ...
    private boolean retryStart() {
        if (!Boolean.TRUE.equals(redisTemplate.opsForValue().setIfAbsent(key, true))) {
            log.info("{} Canal Exists", key);
            return false;
        }
        log.info("{} Canal Running...", key);
        shouldClearTagWhenExit = true;
        config.otter.start();
        log.info("{} Canal Start", key);
        return true;
    }
